pipeline {
    ...
    stages {
        stage('SCM checkout') {
            steps {
                script {
                    git branch: 'master', credentialsId: 'git-cred-smg', url: 'https://github.com/smgowthaman/Jenkins-Terraform-EKS-App-Deploy.git'
                }
            }
        }
            ...
            steps {
                script {
                    withCredentials([[
                        ...
                        credentialsId: 'aws-cred-smg' // Your Jenkins AWS credentials ID
                    ]]) {
        ...
        stage('Deploy on EKS') {
            steps {
                script {
                    withCredentials([[
                        $class: 'AmazonWebServicesCredentialsBinding',
                        credentialsId: 'aws-cred-smg' // Replace with your Jenkins AWS credentials ID
                    ]]) {
                        ...
                        pwd
                        #helm install myrocket /var/lib/jenkins/workspace/project/mynodejsapp-0.1.0.tgz --kubeconfig /opt/kube/config
                        helm upgrade --install myrocket /var/lib/jenkins/workspace/rocket/mynodejsapp-0.1.0.tgz \
